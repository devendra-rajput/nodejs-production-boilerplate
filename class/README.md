...
*A professional-grade Node.js boilerplate built with Object-Oriented Programming principles for enterprise applications.*
...
Both implementations are structured for easy testing:
...
describe('UserController', () => {
  ...
  beforeEach(() => {
    ...
    const mockModel = new MockUserResource();
    const mockResponse = new MockResponseHelper();
    controller = new UserController(mockModel, mockResponse);
  });
  
  it('should create user', async () => {
    const result = await controller.createOne(req, res);
